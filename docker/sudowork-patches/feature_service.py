...
class FeatureService:
    ...
    def get_system_features(cls, is_authenticated: bool = False) -> SystemFeatureModel:
        ...
        # 2026-06-24 Sudowork rebrand: force-enable Dify's existing branding
        # subsystem with Sudowork values, regardless of enterprise license.
        # This drives the web `useDocumentTitle` hook (and similar) to use
        # `branding.application_title` / `branding.favicon` instead of the
        # hard-coded "Dify" fallback, removing the need to ship a fork of
        # the web bundle. Values read from env so ops can adjust without
        # touching code. See docs/plans/2026-06-24-dify-rebrand-to-sudowork.md.
        import os
        if os.environ.get("SUDOWORK_REBRAND_ENABLED", "true").lower() == "true":
            system_features.branding.enabled = True
            system_features.branding.application_title = os.environ.get(
                "SUDOWORK_APP_TITLE", "Sudowork"
            )
            # Use existing logo we bind-mounted; favicon override is optional.
            favicon = os.environ.get("SUDOWORK_FAVICON_URL", "")
            if favicon:
                system_features.branding.favicon = favicon
            workspace_logo = os.environ.get("SUDOWORK_WORKSPACE_LOGO_URL", "")
            if workspace_logo:
                system_features.branding.workspace_logo = workspace_logo
            login_page_logo = os.environ.get("SUDOWORK_LOGIN_PAGE_LOGO_URL", "")
            if login_page_logo:
                system_features.branding.login_page_logo = login_page_logo
