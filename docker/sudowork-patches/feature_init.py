...
class SudoworkConfig(BaseSettings):
    """
    Sudowork integration namespace (controllers/sudowork).
    All endpoints under /sudowork/* are gated by SUDOWORK_INTEGRATION_ENABLED.
    """

    SUDOWORK_INTEGRATION_ENABLED: bool = Field(default=False)
    SUDOWORK_SSO_SECRET: str = Field(default="")
    SUDOWORK_SSO_JTI_TTL_SECONDS: int = Field(default=600)
    SUDOWORK_SYSTEM_SECRET: str = Field(default="")
    SUDOWORK_SYSTEM_TOKEN: str = Field(default="")
    SUDOWORK_ALLOWED_IPS: str = Field(default="")
    SUDOWORK_DEFAULT_ROLE: str = Field(default="admin")
    SUDOWORK_ACCOUNT_EMAIL_DOMAIN: str = Field(default="local.sudowork")
