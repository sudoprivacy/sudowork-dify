...
class SudoworkConfig(BaseSettings):
    """
    Configuration for the SudoWork integration namespace.

    All endpoints under api/controllers/sudowork/ check these settings:
    - SUDOWORK_INTEGRATION_ENABLED gates the whole namespace (default off).
    - SUDOWORK_SSO_SECRET signs the short-lived JWT used for the admin SSO
      handoff coming from sudowork-server.
    - SUDOWORK_SYSTEM_SECRET protects the tenant provisioning endpoint
      (called server-to-server, HMAC-authenticated body).
    - SUDOWORK_SYSTEM_TOKEN is a long-lived bearer accepted on management
      endpoints (apps/datasets CRUD) so sudowork-server can act on behalf
      of a tenant without a user session.
    - SUDOWORK_ALLOWED_IPS optionally pins the system endpoints to the
      sudowork-server egress; comma separated CIDR/IP list.
    """

    SUDOWORK_INTEGRATION_ENABLED: bool = Field(
        description="Enable the SudoWork integration namespace.",
        default=False,
    )

    SUDOWORK_SSO_SECRET: str = Field(
        description="HMAC secret used to verify SSO JWTs issued by sudowork-server.",
        default="",
    )

    SUDOWORK_SSO_JTI_TTL_SECONDS: int = Field(
        description="TTL for storing used JTI nonces (replay window).",
        default=600,
    )

    SUDOWORK_SYSTEM_SECRET: str = Field(
        description="HMAC secret protecting tenant provisioning (server-to-server).",
        default="",
    )

    SUDOWORK_SYSTEM_TOKEN: str = Field(
        description="Bearer token sudowork-server uses to call management endpoints on behalf of a tenant.",
        default="",
    )

    SUDOWORK_ALLOWED_IPS: str = Field(
        description="Optional comma-separated allowlist of source IPs / CIDR ranges for system endpoints.",
        default="",
    )

    SUDOWORK_DEFAULT_ROLE: str = Field(
        description="TenantAccountRole granted to SSO-provisioned SudoWork admins. Must NOT be 'owner'.",
        default="admin",
    )

    SUDOWORK_ACCOUNT_EMAIL_DOMAIN: str = Field(
        description="Synthetic email domain for SSO-provisioned accounts when sudowork-server has no real email.",
        default="local.sudowork",
    )
