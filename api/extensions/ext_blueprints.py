...
def init_app(app: DifyApp):
    ...
    from controllers.sudowork import bp as sudowork_bp
    ...
    if dify_config.SUDOWORK_INTEGRATION_ENABLED:
        # The SSO landing needs CORS+cookies because the browser is redirected
        # here from sudowork-server; the system endpoints are server-to-server
        # so they don't need CORS at all. One CORS block covers both.
        _apply_cors_once(
            sudowork_bp,
            allow_headers=["Content-Type", "Authorization", "X-Sudowork-Tenant", "X-Sudowork-Actor", "X-Sudowork-Signature"],
